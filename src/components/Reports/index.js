...
const AnnualReportsObj = [
    ...
    },
    {
        ...
        description: "2021 Annual Reports",
        pdfLink: "/Images/annualReports2022.pdf"
    }
]
...
class Reports extends Component{
    ...
    render(){
        const {annualButton, financialButton, statuoryButton, carrierGuidanceButton} = this.state;
        return(
            <div className='reportsContainerBg'>
                ...
                <div className='reportsButtonContainer'>
                    <div className='buttonReportsContainer'>
                        <button onClick={()=>{this.setState({annualButton: true, financialButton: false, statuoryButton: false, carrierGuidanceButton: false})}} className={`${annualButton ? 'activeButtonReports' : ''} buttonReports`}>Annual reports</button>
                    </div>
                    <div className='buttonReportsContainer'>
                        <button onClick={()=>{this.setState({annualButton: false, financialButton: true, statuoryButton: false, carrierGuidanceButton: false})}}  className={`${financialButton ? 'activeButtonReports' : ''} buttonReports`}>Financial reports</button>
                    </div>
                    <div className='buttonReportsContainer'>
                        <button onClick={()=>{this.setState({annualButton: false, financialButton: false, statuoryButton: true, carrierGuidanceButton: false})}} className = {`${statuoryButton ? 'activeButtonReports' : ''} buttonReports`}>Statutory document</button>
                    </div>
                    <div className='buttonReportsContainer'>
                        <button onClick={()=>{this.setState({annualButton: false, financialButton: false, statuoryButton: false, carrierGuidanceButton: true})}} className={`${carrierGuidanceButton ? 'activeButtonReports' : ''} buttonReports`}>Carrier guidance</button>
                    </div>
                </div>
                {annualButton ? 
                    <div className='reportsPosterArrange'>
                        {AnnualReportsObj.map((eachDetails)=>{
                                let a;
                                return(
                                    <AnnualReport handleClick = {this.openPdf} key = {eachDetails.id} details = {eachDetails}/>
                                )
                            })}
                    </div>: null}
                {financialButton ? 
